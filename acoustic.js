...
d3.acoustic.circularHeat= function module() {
	 var margin = {
			top: 50,
			right: 50,
			bottom: 50,
	...
	var accessor = function(d) {return d.Average;};
...
data.forEach(function(d){ //d is of form [id,value]
				    var time = d.Time;
					var timeCleaned = time.split(":").join("-");
					var dataID = "segment-" + d.Day + "-" + timeCleaned+"-"+id;
				    
				    d3.select("#"+dataID) //select the group matching the id
				        .attr("fill", color(d.Average));
				});
*/
